...
​	对于应急响应而言我们要收集很多关于系统当前的运行状态，一条条命令敲太麻烦而且不全面，因此我们可以提前编写巡检脚本，需要用到的时候，只需bash一下即可。
...
echo -e "`date "+%Y-%m-%d %H:%M:%S"` 企业平台巡检报告"  | mail -a $RESULTFILE -s "平台巡检报告" <Email>
```
...
# 定义结果文件路径
RESULTFILE="$LOGPATH/HostDailyCheck-$(date +%Y%m%d).txt"
...
function getUserStatus(){
  echo "############################ 用户检查 ############################"
  echo "/etc/passwd 最后修改时间：$(stat /etc/passwd | grep Modify | tr '.' ' ' | awk '{print $2,$3}')"
  echo "特权用户"
  echo "--------"
  ...
  for shell in $(grep -v "/sbin/nologin" /etc/shells); do
    for username in $(grep "$shell" /etc/passwd | awk -F: '{print $1}'); do
      lastlog -u $username | grep -v "Never logged in" | awk '{print $1, $3, $4, $5, $6, $7, $8, $9, $10}'
    done
  done | column -t
  echo "空密码用户"
  echo "----------"
...
function getSSHStatus(){
  echo "############################ SSH检查 #############################"
  echo "服务状态：$(systemctl is-active ssh)"
  echo "SSH协议版本：$(grep Protocol /etc/ssh/sshd_config | awk '{print $2}')"
  echo "是否允许ROOT远程登录"
...
function getSyslogStatus(){
  ...
  echo "/etc/rsyslog.conf"
  grep -v "^#" /etc/rsyslog.conf | grep -v "^\$" | sed '/^$/d' | column -t
}
...
function getchage_file_24h(){
  ...
  find / -name '*.asp' -mtime -1 2>/dev/null
  find / -name '*.php' -mtime -1 2>/dev/null
  find / -name '*.aspx' -mtime -1 2>/dev/null
  find / -name '*.jsp' -mtime -1 2>/dev/null
  find / -name '*.html' -mtime -1 2>/dev/null
  find / -name '*.htm' -mtime -1 2>/dev/null
  find / -name core -exec ls -l {} \; 2>/dev/null
  cat /etc/crontab
...
echo "检查结果保存在文件：$RESULTFILE"

```

​	通过这两个脚本，我们可以快速获取当前系统的基本情况，然后再根据响应的情况做应急响应。
